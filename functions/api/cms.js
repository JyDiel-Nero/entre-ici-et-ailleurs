...
/* Horodatage de publication au format ISO avec décalage horaire de Paris,
   cohérent avec les publishDate existants (ex. 2026-07-04T00:50:00.000+02:00).
   Gère automatiquement l'heure d'été (+02:00) / d'hiver (+01:00). */
function nowIso() {
  const now = new Date();
  ...
  const asUTC = Date.UTC(+parisParts.year, +parisParts.month - 1, +parisParts.day,
    +parisParts.hour, +parisParts.minute, +parisParts.second);
  ...
  return parisParts.year + '-' + parisParts.month + '-' + parisParts.day + 'T'
    + parisParts.hour + ':' + parisParts.minute + ':' + parisParts.second + '.' + ms
    + sign + oh + ':' + om;
}
